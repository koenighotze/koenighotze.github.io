...
In the [last](https://blog.koenighotze.de/2020/12/13/2020-12-13-data-distribution-part2/) article we discussed synchronous calls. The resulting challenges on a technological and organisational level led to surprising results.

This article introduces replication as a pattern for data-integration in microservice landscapes. We will look at the basic concepts and especially at its use in hybrid landscapes. Landscapes, where we want to integrate pre-existing datastores with microservices.


## Classical replication at 10 km

First of all, let’s discuss replication itself. We use replication to increase reliability, allow for fail-over and improve performance. The following sketch illustrates a very simplified way of replicating data.
...
The primary instance is the only one capable of processing and modifications. The primary instance handles all creation, deletion or modification requests. The primary instance processes these requests. Then it forwards the changes to the secondary instances. This setup works best for read-mostly use cases, as data can be read from any of the instances. 
...
Let’s take a concrete example to drive the discussion. Suppose a bank wants to modernize their IT and move towards a microservice landscape. One large SQL database stores the financial transactions of customers. This database is of high value to the enterprise and is the Golden Source. This means that this database contains the "truth" about all data stored within. If the database says you transferred some money from A to B, then this is a fact.
...
- Classical databases often only scale vertically (TODO LINK). There is an upper limit to the number of clients such a database can serve at the same time.
...
The Change-Data-Capture (CDC) framework hooks into a source database. The framework captures all changes to the data - hence the name. Finally, the framework transforms and write the data to a target database. One example technology for this use case is [Kafka](https://kafka.apache.org/). The following illustration visualizes this approach.

![Change-Data-Capture](/assets/images/microservices-data-3/cdc-pipeline-white.png)

We hook into our source database for example with [Kafka Connect](https://docs.confluent.io/platform/current/connect/index.html) and [Debezium](https://debezium.io/). Debezium reads the database’s transaction log (TX Log). Debezium forwards changes to the transaction log to Kafka topics. The microservices (MS) consume the data from the topics and fill their own databases (DB) as needed. We can optimize the microservice-databases for the respective use case. For example, one microservice might need a Postgres whereas another needs a Redis.
...
The CDC pipeline (here: Kafka) processes all entries of the transaction log (TX Log). It stores each entry in Kafka-topics and forwards it to the receiving services. These services in turn manifest their own local view of those entries.
...
We can also transform and enrich the data as part of the data replication process. Let’s use as a trivial example; again financial transactions. The next illustration depicts a pipeline.  
...
Microservices use only the business databases. These are derived from the raw data and are optimized for the specific use cases. The business database could be optimized and contain a denormalized view of the data. New business databases can be added as new use cases arise. 

## Implications

Change-data-capture and transformation pipelines are both valid approaches. Both help to move from existing system landscapes towards a more flexible architecture. We can adopt a microservice landscape without any modification to the existing assets. The microservices each end up with their own optimized data-store. This decouples the development teams and increases agility.  

Having Kafka and similar frameworks adds to the development complexity. Even so, 
...
We hook into the source database (Golden Source) again with Kafka Connect and Debezium (I). Kafka topics store transaction log entries as events. The microservice consumes the topics it needs (II). Afterwards, it manifests a local view in its own local business database (III). 

If we want to read financial transactions, we need to query the local business database. The microservice ownes the business database. In the following illustration, a caller sends a GET request to the microservice (I). The microservice queries the optimized local database (II) and answers the query.
...
Updating the message log and the transaction table (TX) happens as one transaction (II and III). Both are part of the same database, so a single local transaction is enough.
The microservice can return the result to the caller and return the result.

Now we get to the tricky part. Handling the message log. Often the API triggers some process side-effects besides to updating the Golden Source. For example, calling out to other APIs or sending messages. 

The following diagram explores the communication flow. 

TODO DIAGRAM

The Outbox Handler polls the message log table or subscribes to changes to it (I). It reads the data and calls the API (II). If calling the API was successful, then the handler marks message log entry as `done`. Otherwise the Outbox Handler retries the operation. If all fails, the handler marks the entry as `not processable`. 

In such cases, other mitigation strategies come into place. But this is outside of our discussion.

Suppose the API call was successful. Next, among other things, the API call updates the Golden Source (III). This triggers the CDC pipeline. The CDC component captures the new data added to the Golden Source’s transaction log. Afterwards this data ends up in the Kafka topics (IV). The consuming microservice receives that data (V). Finally, the microservice updates the business database. The database now reflects the state of the Golden Source, too (VI). We can insert the sound of a fanfare at this point.

We have omitted many technical details. Still, the complexity of this pattern should stand out. There are many things that could go wrong at any point. A solid solution must find mitigation strategies for each error case. 

Even so, the eventual consistent character of this architecture does not go away. The new data stored in the business database does not reflect the Golden Source data right away. The time delay may or may not be an issue for the concrete use case. But we need to be aware of it, and should analyse the impact of it. The same holds for the topic of data governance. The patterns of this article lead to data replication, thus to storing the same data in many places. Depending on the regulatory requirements, we need to control which parts of the system landscape can use which data. This has to be set in place right from the beginning. Refactoring data governance controls into an existing landscape can be very challenging.
...
Moving towards modern architectures without any major refactoring of existing systems is possible. We are able to leverage and use so-called legacy systems without any direct extra cost of doing so.

We do not change existing systems. So we end up with the "old”,""legac"” system landscape and the new microservice landscape. Complexity and cost increase. We need more engineers. We need more infrastructure. And so on. 

But, we must not confuse this with [event-sourcing](https://martinfowler.com/eaaDev/EventSourcing.html) or an [event driven architecture](https://en.wikipedia.org/wiki/Event-driven_architecture). It can be the first step into those areas, but only the first. We are considering technical events a la _"Row x in Table y has changed in values A, D, F"_. This is different to saying _SEPA Transaction executed_. And we have to to deal with eventual consistency. There is no way of avoiding this.

In conclusion, we need to check the advantages and implications of the approaches. There cannot be a best answer. We need to consider the concrete requirements and use cases. These determine if this approach is a good fit for our challenge and strategy.
